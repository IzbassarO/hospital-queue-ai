...
`laptop` defaults to a conservative CPU budget and one trial/process. `overnight` increases bounded utilization.
...
period. One local writer owns a run via `.run.lock`; normal exits, exceptions and Ctrl+C release it. Hard-kill locks
are never removed automatically and require `make train ARGS="--recover-stale-lock <run-id>"` after diagnosis.
