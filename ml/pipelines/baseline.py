...
def main() -> int:
        ...
        math.ceil(
            one(
                """SELECT quantile_cont(
                               date_diff('day', registration_date, greatest(resolution_date, registration_date)), 0.9)
    ...
    L += [
        "# Baseline — planned hospitalization queues (descriptive, no models)",
        "",
    ...
    for key, src in manifest["sources"].items():
        ...
        staged = (
            fmt(src["rows"])
            if "rows" in src
            else f"not staged (only {len(src.get('region_codes', []))} region codes read)"
        )
        rows.append([key.replace("_", " "), staged, len(src.get("files", [])), skipped])
    L += table(["source", "rows staged", "files used", "files skipped"], rows)
    L += [
        "All 6 parts of dataset 3 were valid in this run."
        if not manifest["sources"]["dataset_3"]["skipped"]
    ...
    L += [
        "Postgres load: "
        f"{'yes — row counts verified per table' if pg.get('loaded') else 'no (' + str(pg.get('reason')) + ')'}.",
        "",
    ]
    L += [
        f"- **Window**: registrations {p.window_start} → {p.window_end} (daily aggregates cover the same dates).",
        "- **Wait** (`wait_days`) = calendar days from registration to hospitalization, floored at 0; hospitalized "
        "referrals only.",
        ...
        f"- **Queue warm-up**: the data has no referrals registered before {p.window_start}, so the reconstructed "
        "queue "
        f"starts near zero. 90% of referrals leave the queue within **{warmup_days} days**, so queue levels are "
        "treated "
        ...
        f"- **Throughput proxy** = ERSB `discharged_total` / {ASSUMED_ERSB_DAYS} (assumption: the ERSB snapshot "
        "covers one "
    ...
    region_wait = q(
        wait_sql.format(
            key="f.region_code",
            label="r.region_name",
            join="LEFT JOIN dim_region r ON r.region_code = f.region_code",
            where="",
        )
    ...
    top_profiles = [
        r[0]
        for r in q(
            "SELECT profile_code FROM fact_referral WHERE outcome = 'hospitalized' GROUP BY 1 ORDER BY count(*) DESC "
            "LIMIT 15"
        )
    ]
    prof_wait = q(
    ...
    nat_ref = one(
        "SELECT count(*), count(*) FILTER (WHERE outcome='refused'), avg((outcome='refused')::INT) FROM fact_referral"
    )
    L += [
        "## 3. Refusal rate",
        "",
        "### 3a. By region of origin",
        "",
    ...
    L += table(
        ["code", "region", "referrals", "refused", "refusal rate", "open", "admission-unit refusals (ds3)"],
        [
            [r[0], cut(r[1], 40), fmt(r[2]), fmt(r[3]), pct(r[4]), fmt(r[5]), fmt(adm_region.get(r[0]))]
            for r in ref_region
        ]
        + [
            [
    ...
    ref_prof = q(
        ref_sql.format(
            ...
            label="pr.profile_name",
            join="LEFT JOIN dim_profile pr ON pr.profile_code = f.profile_code",
            where="WHERE f.profile_code IN (SELECT unnest($codes))",
        )
        + " ORDER BY rate DESC",
        {"codes": top_profiles},
    )
    ...
    run(
        """CREATE TEMP TABLE org_load AS
           ...
           GROUP BY ALL""",
        {"days": ASSUMED_ERSB_DAYS, "steady": steady_start},
    )
    top_load = q(
        """SELECT org_code, org_name, region_name, mean_queue, throughput, mean_queue / throughput ratio,
                           discharged_total, match_method, n_matched_org_codes
                    FROM org_load WHERE throughput >= $min_tp
                    ORDER BY ratio DESC LIMIT 20""",
        {"min_tp": MIN_THROUGHPUT_PER_DAY},
    )
    ...
    L += [
        "## 4. Top-20 hospitals by queue relative to throughput",
        "",
        ...
        f"queue. Only hospitals matched to ERSB with ≥ {MIN_THROUGHPUT_PER_DAY:g} discharge/day ({n_eligible} "
        "hospitals). "
    ...
    L += table(
        ["org", "hospital", "region", "mean queue", "throughput/day", "ratio", "ERSB discharged", "match"],
        [
            [
                r[0],
                short_org(r[1], 60),
                ...
                fmt(r[5]),
                fmt(r[6]),
                r[7] + (f" (ERSB row shared by {r[8]} codes)" if r[8] and r[8] > 1 else ""),
            ]
            for r in top_load
        ],
    )
    ...
    L += table(
        ["code", "region", "mean queue (all hospitals)", "mean queue (ERSB-matched)", "throughput/day", "ratio"],
        [
            [r[0], cut(r[1], 35), fmt(r[4]), fmt(r[2]), fmt(r[3]), fmt(r[2] / r[3] if r[3] else None)]
            for r in region_load
        ],
    )

    # -------------------------------------------------------- weekly national
    weekly = q("""WITH nat AS (
                      SELECT date, sum(registrations) regs, sum(hospitalizations) hosp, sum(refusals) refs,
                  ...
                  SELECT CAST(date_trunc('week', nat.date) AS DATE) wk, min(nat.date), max(nat.date),
                         count(*) AS n_days,
    ...
    L += table(
        ...
        [
            [
                f"{r[0]}{' (partial)' if r[3] < 7 else ''}{' *' if r[2] < steady_start else ''}",
                r[3],
                fmt(r[4]),
                fmt(r[5]),
                fmt(r[6]),
    ...
    run(
        """CREATE TEMP TABLE org_missing AS
           ...
           lag AS (SELECT org_code, date_diff('day', registration_date, greatest(resolution_date, registration_date))
                   AS L
                   FROM fact_referral WHERE resolution_date IS NOT NULL)
           SELECT l.org_code, w.wk, sum(greatest(l.L - w.t, 0)) / $window_days AS missing
           FROM lag l CROSS JOIN wk w GROUP BY ALL""",
    ...
    growth = q(
        """WITH w AS (
                  ...
                  a AS (SELECT org_code, regr_slope(-missing, epoch(wk) / 604800) artifact
                        FROM org_missing GROUP BY org_code),
                  ...
                  FROM x JOIN dim_organization o USING (org_code)
                  LEFT JOIN dim_region r ON r.region_code = o.region_code
                  WHERE x.mean_q >= $minq AND x.steps > 0 AND x.ups >= 0.75 * x.steps
                    AND x.excess / x.mean_q >= $min_rel
                  ORDER BY rel DESC""",
        {"minq": MIN_MEAN_QUEUE_FOR_GROWTH, "min_rel": MIN_EXCESS_GROWTH_PER_WEEK},
    )
    n_big = one(
        "SELECT count(*) FROM (SELECT org_code FROM org_weekly GROUP BY 1 HAVING avg(q) >= $minq)",
        {"minq": MIN_MEAN_QUEUE_FOR_GROWTH},
    )[0]
    L += [
        "## 6. Hospitals whose queue grows week over week",
        "",
        f"Full ISO weeks after warm-up: {weeks[0] if weeks else '—'} … {weeks[-1] if weeks else '—'} ({len(weeks)} "
        "weeks). "
        ...
        f"the window itself. Nationally the observed queue grows by {fmt(nat_obs, 0)}/week and the missing history "
        "alone "
        f"is expected to produce {fmt(nat_art, 0)}/week"
        + (
            " — **the national growth is fully explained by the artifact**."
            if nat_art >= nat_obs
            else f" — {pct(1 - nat_art / nat_obs)} of the national growth is not explained by it."
        ),
        "",
        f"Listed: hospitals with mean queue ≥ {MIN_MEAN_QUEUE_FOR_GROWTH} ({n_big} hospitals), weekly mean queue "
        "rising "
    ...
    def check(name, value, expected=None, ok=None, info=False):
        ...
        checks.append(
            [
                name,
                fmt(value) if not isinstance(value, str) else value,
                "" if expected is None else (fmt(expected) if not isinstance(expected, str) else expected),
                status,
            ]
        )

    check(
        "min queue_length (agg_daily_hospital_profile)",
        one("SELECT min(queue_length) FROM agg_daily_hospital_profile")[0],
        ">= 0",
        ok=one("SELECT min(queue_length) FROM agg_daily_hospital_profile")[0] >= 0,
    )
    check(
        "min queue_length (agg_daily_region_profile)",
        one("SELECT min(queue_length) FROM agg_daily_region_profile")[0],
        ">= 0",
    ...
    n_fact, n_h, n_r, n_o, n_conf = one("""SELECT count(*), count(*) FILTER (WHERE outcome='hospitalized'),
        count(*) FILTER (WHERE outcome='refused'), count(*) FILTER (WHERE outcome='open'),
        count(*) FILTER (WHERE outcome_conflict)
        FROM fact_referral""")
    ...
    exp = one(
        """SELECT count(*) FILTER (WHERE registration_date BETWEEN $start AND $end),
    ...
    ragg = one(
        "SELECT sum(registrations), sum(hospitalizations), sum(refusals), sum(queue_length) FROM "
    ...
    q_end_fact = one(
        """SELECT count(*) FROM fact_referral WHERE registration_date <= $end
                        AND (resolution_date IS NULL OR resolution_date > $end)""",
        W,
    )[0]
    ...
    ersb_cov = one("""SELECT avg((ersb_id IS NOT NULL)::INT),
                             sum(n_referrals) FILTER (WHERE ersb_id IS NOT NULL) / sum(n_referrals),
                             count(*) FILTER (WHERE match_method = 'fuzzy')
                      FROM dim_organization""")
    check(
        "hospitals matched to ERSB (share of org codes / of referrals)",
        f"{pct(ersb_cov[0])} / {pct(ersb_cov[1])}",
        info=True,
    )
    ...
    check(
        "dataset 3 rows without org_code (hospital not in dataset 1)", f"{pct(ds3[0])} ({ds3[2]} hospitals)", info=True
    )
    ...
    if pg.get("loaded"):
        ...
        check(
            "Postgres row counts = Parquet row counts (all tables)",
            "mismatch: " + ", ".join(mism) if mism else "equal",
            "equal",
            ok=not mism,
        )
    L += ["## 7. Sanity checks", ""]
    ...
    L += ["## 8. Findings", ""]
    L += [
        "1. **Regional imbalance is large.** For patients who actually waited (not admitted the same day), the "
        "median wait "
        ...
        f"{len(growth)} of {n_big} hospitals with a meaningful queue (mean ≥ {MIN_MEAN_QUEUE_FOR_GROWTH}) still grow "
        "by "
        f"≥ {pct(MIN_EXCESS_GROWTH_PER_WEEK)} of their queue per week and rose in most weeks"
        + (
            ": "
            + "; ".join(
                f"{short_org(r[1], 60)} ({r[2]}, weekly mean {fmt(r[4], 0)} → {fmt(r[5], 0)})" for r in growth[:3]
            )
            if growth
            else ""
        )
        + ". These are the natural first candidates for queue-growth alerts.",
        f"4. **Strongest signal for the product**: queue relative to capacity. Across {n_eligible} ERSB-matched "
        "hospitals "
        ...
        "(section 4) shows where queues are large relative to what the hospital actually discharges — together with "
        "wait "
        ...
        + (
            ...
            "queue actually grew in Q1 2025. "
            if nat_art >= nat_obs
            else f"only {pct(nat_art / nat_obs)} of that rise is expected from referrals missing before 2025. "
        )
        ...
        f"Region codes {', '.join(amb) or '—'} have an ambiguous vote (cross-region patient flows) and should be "
        "confirmed in "
