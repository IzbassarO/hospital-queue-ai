...
class Panel:
    ...
    def subset(self, mask: np.ndarray) -> "Panel":
        return Panel(
            self.dates,
            self.meta[mask].reset_index(drop=True),
            self.registrations[mask],
            self.hospitalizations[mask],
            self.queue[mask],
        )
...
def load_panels(con: duckdb.DuckDBPyConnection) -> tuple[Panel, Panel]:
    ...
    hm, hd, ha = _dense(
        con,
        """SELECT date, org_code, region_code, profile_code, registrations, hospitalizations, queue_length
                                FROM agg_daily_hospital_profile""",
        ["org_code", "region_code", "profile_code"],
    )
    hm.insert(0, "level", "hospital")
    hm.insert(0, "series_id", "hp:" + hm["org_code"] + ":" + hm["profile_code"])
    rm, rd, ra = _dense(
        con,
        """SELECT date, region_code, profile_code, registrations, hospitalizations, queue_length
                                FROM agg_daily_region_profile""",
        ["region_code", "profile_code"],
    )
...
def concat(a: Panel, b: Panel) -> Panel:
    return Panel(
        a.dates,
        pd.concat([a.meta, b.meta], ignore_index=True),
        np.vstack([a.registrations, b.registrations]),
        np.vstack([a.hospitalizations, b.hospitalizations]),
...
def build_rows(
    ...
    for t in cutoffs:
        ...
        for w in rolling:
            j0 = t - w + 1
            cols[f"roll_mean_{w}"] = np.repeat(
                (csum[:, t + 1] - csum[:, j0]) / w if j0 >= 0 else np.full(S, np.nan), n_h
            )
