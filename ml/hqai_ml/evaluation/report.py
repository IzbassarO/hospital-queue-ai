...
def compare_table(rows: list[dict], columns: list[tuple]) -> list[str]:
    ...
    return table(["model"] + [c[0] for c in columns],
                 [[r["model"]] + [col[i] for col in cells] for i, r in enumerate(rows)])
...
def _load(settings: IngestSettings, name: str) -> dict | None:
    try:
        ...
        return None
    return {"version": path.name, "meta": store.load_json(path, "meta.json"), "metrics": store.load_json(path, "metrics.json"),
...
def section_data(cfg: ModelConfig, arts: dict, settings: IngestSettings) -> list[str]:
    ...
    if arts.get("wait_time"):
        ...
        rows.append(["A wait time", p["definition"], f(p["train_rows"]), f(p["test_rows"]), "wait_days (trained on log1p)"])
    if arts.get("refusal_risk"):
        p = arts["refusal_risk"]["metrics"]["population"]
        rows.append(["B refusal risk", p["definition"], f(p["train_rows"]), f(p["test_rows"]),
    ...
    if arts.get("load_forecast"):
        ...
        rows.append(["C load forecast", "daily series from agg_daily_hospital_profile / agg_daily_region_profile",
                     f"{f(sc['hospital_profile_modelled'])} + {f(sc['region_profile'])} series",
                     "3 rolling origins × 14 days", "daily registrations, daily hospitalizations"])
    L += table(["model", "population", "train", "test", "target"], rows)
    L += [f"- **Temporal split**: train on registrations {s.train_start} … {s.train_end}, test on {s.test_start} … "
          ...
          f"- Model C: rolling origins {', '.join(str(o) for o in cfg.load_forecast.backtest_origins)} (first forecast day; "
...
def section_features() -> list[str]:
    L = ["## 1. Features and leakage review", "",
         "d = registration date. \"end of day d−1\" means the value uses only events up to the day before registration.", ""]
    L += table(["feature", "used in", "known at", "note"],
               [[f"`{k}`", v[0], v[1], v[2]] for k, v in REFERRAL_FEATURES.items()])
...
def _examples(ex: list[dict], unit: str) -> list[str]:
    L = ["Each factor's effect is its SHAP value converted to " + ("days" if unit == "days" else "percentage points")
    ...
    for e in ex:
        actual = "—" if e["actual"] is None else (f"{e['actual']:.0f} days" if unit == "days" else ("refused" if e["actual"] else "hospitalized"))
        pred = f"{e['prediction']:.1f} days" if unit == "days" else pct(e["prediction"])
        L += [f"**Referral {e['referral_id']}** (registered {e['registration_date']}, profile {e['profile_code']}, "
...
def _shap_table(shap: dict, unit: str) -> list[str]:
    L = [f"Mean |SHAP| on {f(shap['sample_size'])} test referrals, in model units ({unit}); additivity check "
         f"max |base + ΣSHAP − prediction| = {shap['additivity_max_abs_error']:.1e}.", ""]
    return L + table(["feature", "mean abs SHAP", "share"],
...
def section_wait(a: dict) -> list[str]:
    ...
    L = [f"## 2. Model A — wait time (version {a['version']}, {m['best_iteration']} trees)", "",
         f"Population: {m['population']['definition']}. Median wait: train {m['population']['train_median_wait']:.0f} days, "
         f"test {m['population']['test_median_wait']:.0f} days.", "", "### 2.1 Test set, model vs baselines", ""]
    L += compare_table(m["overall"], [("MAE, days", "mae", "min", f), ("median AE", "median_ae", "min", f),
                                      ("WAPE", "wape", "min", pct), ("within ±7 days", "within_7d", "max", pct),
                                      ("Spearman", "spearman", "max", lambda v: f(v, 3))])
    L += ["### 2.2 By patient region (test)", ""]
    L += table(["region", "n", "median actual", "MAE model", "MAE B1", "MAE B2", "MAE B3", "WAPE model", "WAPE B3",
                "±7d model", "±7d B3", "Spearman model", "Spearman B3"],
               [[cut(disp.get("region", {}).get(r["segment"], r["segment"]), 30), f(r["n"]), f(r["median_actual"], 0),
                 *bold_min([r["mae_model"], r["mae_b1"], r["mae_b2"], r["mae_b3"]], f), pct(r["wape_model"]), pct(r["wape_b3"]),
                 pct(r["within_7d_model"]), pct(r["within_7d_b3"]), f(r["spearman_model"], 3), f(r["spearman_b3"], 3)]
                for r in m["by_region"]])
    L += ["### 2.3 By top-10 profiles (test volume)", ""]
    L += table(["profile", "n", "median actual", "MAE model", "MAE B1", "MAE B2", "MAE B3", "WAPE model", "WAPE B3",
                "±7d model", "±7d B3", "Spearman model", "Spearman B3"],
               [[cut(f"{r['segment']} {disp.get('profile', {}).get(r['segment'], '')}", 40), f(r["n"]), f(r["median_actual"], 0),
                 *bold_min([r["mae_model"], r["mae_b1"], r["mae_b2"], r["mae_b3"]], f), pct(r["wape_model"]), pct(r["wape_b3"]),
                 pct(r["within_7d_model"]), pct(r["within_7d_b3"]), f(r["spearman_model"], 3), f(r["spearman_b3"], 3)]
    ...
    L += table(["variant", "MAE, days", "median AE", "WAPE", "within ±7 days", "Spearman"],
               [[r["model"], f(r["mae"]), f(r["median_ae"]), pct(r["wape"]), pct(r["within_7d"]), f(r["spearman"], 3)]
                for r in m["ablation_planned_lag"]])
    L += ["The planned date on its own predicts the wait almost exactly. That is only possible if `planned_dt` is set "
          "or corrected once the admission is scheduled, so it is not a registration-time feature and stays excluded "
          "until the data owner confirms when it is filled.", "", "### 2.6 Example explanations", ""]
    return L + _examples(m["examples"], "days")


def section_refusal(b: dict) -> list[str]:
    m, disp = b["metrics"], b["display"]
    L = [f"## 3. Model B — refusal risk (version {b['version']}, {m['best_iteration']} trees)", "",
         f"Population: {m['population']['definition']}. Refusal rate: train {pct(m['population']['train_refusal_rate'])}, "
         f"test {pct(m['population']['test_refusal_rate'])}.", "", "### 3.1 Test set, model vs baselines", "",
         "`top 10%` = the 10% of test referrals with the highest predicted risk.", ""]
    L += compare_table(m["overall"], [("ROC-AUC", "roc_auc", "max", lambda v: f(v, 3)),
    ...
    L += table(["bin", "n", "predicted range", "mean predicted", "observed", "B3 mean predicted", "B3 observed"],
               [[c["bin"] + 1, f(c["n"]), f"{c['p_min']:.3f} – {c['p_max']:.3f}", pct(c["mean_pred"]), pct(c["observed"]),
                 pct(c3["mean_pred"]), pct(c3["observed"])] for c, c3 in zip(m["calibration_model"], m["calibration_b3"])])
    seg_header = ["n", "refusal rate", "mean predicted", "ROC-AUC model", "ROC-AUC B3", "PR-AUC model", "PR-AUC B3", "Brier model", "Brier B3"]

    def seg_row(r):
        return [f(r["n"]), pct(r["refusal_rate"]), pct(r["mean_pred_model"]),
    ...
    L += table(["region"] + seg_header, [[cut(disp.get("region", {}).get(r["segment"], r["segment"]), 30)] + seg_row(r) for r in m["by_region"]])
    L += ["### 3.4 By top-10 profiles (test volume)", ""]
    L += table(["profile"] + seg_header, [[cut(f"{r['segment']} {disp.get('profile', {}).get(r['segment'], '')}", 40)] + seg_row(r)
                                         for r in m["by_profile"]])
    L += ["### 3.5 SHAP global importance", ""] + _shap_table(m["shap"], "log-odds")
    L += ["### 3.6 Ablation: the excluded `planned_lag_days`", ""]
    L += table(["variant", "ROC-AUC", "PR-AUC", "Brier", "precision top 10%", "recall top 10%"],
               [[r["model"], f(r["roc_auc"], 3), f(r["pr_auc"], 3), f(r["brier"], 4), pct(r["precision_top"]), pct(r["recall_top"])]
                for r in m["ablation_planned_lag"]])
    L += ["A missing planned date alone is a strong refusal signal — consistent with `planned_dt` being filled only when "
          "the hospital accepts the referral. Excluded for the same reason as in Model A.", "", "### 3.7 Example explanations", ""]
...
def _forecast_rows(rows, keys):
    ...
    for r in rows:
        ...
        out.append([*(r[k] for k in keys), f(r["n"]), f(r["actual_total"], 0), *w, f(r["mae_model"]), f(r["mae_seasonal_naive"])])
    return out
...
def section_load(c: dict, display: dict) -> list[str]:
    m = c["metrics"]
    sc = m["series"]
    hdr_tail = ["n", "actual total", "WAPE model", "WAPE seasonal naive", "WAPE mean 28d", "WAPE mean 7d", "MAE model", "MAE seasonal naive"]
    L = [f"## 4. Model C — load forecast (version {c['version']})", "",
         f"Series: {f(sc['hospital_profile_modelled'])} of {f(sc['hospital_profile_total'])} hospital × profile series have "
         ...
         f"All {f(sc['region_profile'])} region × profile series are modelled. One global LightGBM (Poisson) model per target.", "",
         "### 4.1 Does the model beat the seasonal-naive baseline? (pooled over 3 origins × 14 days)", ""]
    L += table(["target", "series level", "WAPE model", "WAPE seasonal naive", "WAPE mean 28d", "WAPE mean 7d",
                "beats seasonal naive", "beats all baselines", "origin × bucket cells beating seasonal naive"],
               [[r["target"], r["eval_level"], *bold_min([r["wape_model"], r["wape_seasonal_naive"], r["wape_mean_28d"], r["wape_mean_7d"]], pct),
                 f(r["beats_seasonal_naive"]), f(r["beats_all_baselines"]), f"{r['cells_beating_seasonal_naive']} / {r['cells']}"]
    ...
    if cmp:
        change = "; ".join(
            ...
            + [f"holidays removed: {', '.join(cmp['holidays_removed'])}"] * bool(cmp["holidays_removed"])) or "no holiday change"
        cells = "; ".join(f"{r['target']} {r['eval_level']} {pct(r['wape_model_before'])} → {pct(r['wape_model_after'])}"
                          for r in sorted(cmp["pooled"], key=lambda r: (r["target"], r["eval_level"])))
        L += [f"**Change vs previous version `{cmp['previous_version']}` ({change}).** Pooled model WAPE before → after: {cells}.", ""]
    ...
    for origin in m["backtest_origins"]:
        ...
        L += table(["target", "series level", "horizon"] + hdr_tail, _forecast_rows(rows, ["target", "eval_level", "bucket"]))
    L += ["### 4.3 Pooled over origins, by horizon bucket", ""]
    L += table(["target", "series level", "horizon"] + hdr_tail, _forecast_rows(m["pooled_by_bucket"], ["target", "eval_level", "bucket"]))
    ...
    L += [f"### 4.5 Top-{len({r['org_code'] for r in m['by_top_hospital']})} hospitals by train-period registrations "
    ...
    qrows = []
    for r in m["queue"]:
        qrows.append([r["origin"], r["eval_level"], r["bucket"], f(r["n"]), *bold_min([r["wape_model"], r["wape_last_value"]], pct),
                      pct(r.get("bias_model")), pct(r.get("bias_last_value"))])
    L += table(["origin", "series level", "horizon", "n", "WAPE queue forecast", "WAPE last value",
                "bias queue forecast", "bias last value"], qrows)
    L += ["`bias` = Σ(forecast − actual) / Σactual; positive means the forecast queue is too long.", ""]
    if m.get("calendar_check"):
        L += ["### 4.6a Calendar check: test-period weekdays with near-zero volume", "",
              "National registrations below 20% of the median for the same weekday. Days without a holiday flag are "
              "missing from `load_forecast.holidays` in `ml/configs/models.yaml`, so the model forecasts them as normal days.", ""]
        L += table(["date", "weekday", "registrations", "weekday median", "holiday flag in config"],
    ...
    for tgt, imp in m["gain_importance"].items():
        ...
        L += [f"**{tgt}**", ""] + table(["feature", "gain share"], [[f"`{x['feature']}`", pct(x["gain"] / total)] for x in imp[:10]])
    return L
...
def section_findings(arts: dict) -> list[str]:
    ...
    if a:
        ...
        not_best = [k for k, d in metric_dirs.items()
                    if any(r[k] is not None and (r[k] < lg[k] if d == "min" else r[k] > lg[k])
                           for r in m["overall"] if r["model"] != "LightGBM")]
        verdict = ("beats all three baselines on every metric" if not not_best else
                   f"beats all three baselines on {', '.join(k for k in metric_dirs if k not in not_best)}, "
                   f"but not on {', '.join(not_best)}")
        L += [f"**Wait time (A).** The model {verdict}: MAE {lg['mae']:.1f} days vs "
              f"{b3['mae']:.1f} for the best baseline (hospital × profile median, −{100 * (1 - lg['mae'] / b3['mae']):.0f}%), "
              f"Spearman {lg['spearman']:.2f} vs {b3['spearman']:.2f}. The gain is modest: most of what is predictable is "
              f"*which hospital and profile* the referral goes to (top SHAP features: {', '.join(f'`{t}`' for t in top)}), "
              ...
              + ". Hardest profiles: " + "; ".join(f"{prof.get(r['segment'], r['segment'])} ({r['mae_model']:.1f})" for r in hard_prof)
              + ". " + (f"The model does not beat B3 on MAE in: {', '.join(lost)}." if lost else
                        "It beats B3 on MAE in every region."), ""]
    if b:
        m = b["metrics"]
        ov = {r["model"]: r for r in m["overall"]}
        ...
        low = sorted([r for r in m["by_region"] if r["roc_auc_model"] is not None], key=lambda r: r["roc_auc_model"])[:3]
        lost = [reg.get(r["segment"], r["segment"]) for r in m["by_region"] if (r["roc_auc_model"] or 0) <= (r["roc_auc_b3"] or 0)]
        L += [f"**Refusal risk (B).** ROC-AUC {lg['roc_auc']:.3f} vs {b3['roc_auc']:.3f} (B3), PR-AUC {lg['pr_auc']:.3f} vs "
              ...
              "Weakest ranking by region: " + "; ".join(f"{reg.get(r['segment'], r['segment'])} (AUC {r['roc_auc_model']:.3f})" for r in low)
              + ". " + (f"The model does not beat B3 on ROC-AUC in: {', '.join(lost)}." if lost else
                        "It beats B3 on ROC-AUC in every region.")
    ...
    if c:
        ...
        for r in m["beats_seasonal_naive"]:
            ...
            lines.append(f"- {r['target']}, {r['eval_level']}: model {verdict} seasonal naive "
                         f"(WAPE {pct(r['wape_model'])} vs {pct(r['wape_seasonal_naive'])}; mean 28d {pct(r['wape_mean_28d'])}, "
                         f"mean 7d {pct(r['wape_mean_7d'])}; {r['cells_beating_seasonal_naive']}/{r['cells']} origin × bucket cells).")
        q = m["queue"]
        q_wins = sum(r["wape_model"] < r["wape_last_value"] for r in q)
        biases = [r["bias_model"] for r in q if r.get("bias_model") is not None]
        strongest = {}
        for r in m["beats_seasonal_naive"]:
            b = min(("seasonal naive", r["wape_seasonal_naive"]), ("mean 28d", r["wape_mean_28d"]), ("mean 7d", r["wape_mean_7d"]),
                    key=lambda x: x[1])
        ...
        def calendar_note(r):
            ...
            if flagged:
                ...
                parts.append(f"it contains the holiday days {', '.join(flagged)} (flagged, but the model trained for this "
                             f"origin has seen only {len(seen)} holiday days to learn their effect from)")
            if unflagged_days:
                parts.append(f"it contains {', '.join(unflagged_days)}, a non-working weekday missing from the holiday list")
            return "; ".join(parts)
        ...
        for c in calendar:
            ...
            hit = [r for r in region_rows if c["date"] in cell_days(r) and r["target"] == "registrations" and r["bucket"] == "1–7"]
            for r in hit:
                others = [x["wape_model"] for x in region_rows if x["target"] == r["target"] and x["bucket"] == r["bucket"]
                          and x["origin"] != r["origin"] and not any(d["date"] in cell_days(x) for d in calendar)]
                unflagged_cells.append((c["date"], r, min(others) if others else None))
        L += ["**Load forecast (C).** Verdict against the seasonal-naive baseline (same weekday last week):", ""] + lines + [
              "",
              f"The strongest baseline at every series level is {', '.join(strongest_names)}: daily referral counts have a "
              "very strong weekday pattern (weekends are near zero), which flat 7- and 28-day means ignore. The model "
              "learns essentially the same thing (`same_weekday_last` and `target_weekday` carry most of the gain) and adds "
              "level information from the rolling means, which is why its margin over seasonal naive is small at region "
              ...
              + "".join(f" Origin {r['origin']}, registrations, horizon {r['bucket']} contains {day}, a non-working weekday "
                        f"missing from the configured holiday list (calendar check 4.6a): WAPE {pct(r['wape_model'])} vs "
                        f"{pct(best)} for the same horizon at an origin without special days. Adding transferred days off to the "
                        "calendar is the cheapest improvement available."
                        for day, r, best in unflagged_cells),
              f"The derived queue forecast is **worse than keeping the last known queue** in {len(q) - q_wins} of {len(q)} "
              f"origin × level × horizon cells. It is biased upward in {sum(b > 0 for b in biases)} of {len(biases)} cells "
              f"(bias {pct(min(biases))} to {pct(max(biases))}) "
              "because the formula subtracts only hospitalizations while refusals also leave the queue, and summing 14 "
              "noisy daily forecasts adds variance that a flat forecast does not have. As specified it should not be shown "
...
def section_limitations() -> list[str]:
    return ["## 6. Limitations", "",
            ...
            "- **Holidays**: the test month contains Nauryz (21–25 March) and the transferred day off 10 March (both in the "
...
def write_report(settings: IngestSettings, cfg: ModelConfig) -> Path:
    ...
    L = ["# Models — wait time, refusal risk, load forecast", "",
