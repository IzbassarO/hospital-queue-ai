...
def fit_model(name: str, train: pd.DataFrame, test: pd.DataFrame, y_train: np.ndarray, features: list[str],
              objective: dict, cfg: ModelConfig, display: dict) -> FitResult:
    ...
    booster, best = fit_early_stopped(X, y_train, holdout, params, categorical,
                                      cfg.early_stopping.max_rounds, cfg.early_stopping.patience)
...
def examples(model: ReferralModel, test: pd.DataFrame, pred: np.ndarray, actual_col: str, cfg: ModelConfig) -> list[dict]:
