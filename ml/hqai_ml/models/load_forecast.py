...
def split_series(hospital: Panel, region: Panel, cfg: ModelConfig) -> dict:
    ...
    region_pos = {(r, p): i for i, (r, p) in enumerate(zip(region.meta["region_code"], region.meta["profile_code"]))}
    shares = {}
...
def fit_target(panel: Panel, target: str, last_known: int, categories: dict, cfg: ModelConfig) -> lgb.Booster:
    ...
    return lgb.train(lgb_params(cfg), lgb.Dataset(X, rows["y"].to_numpy(float), categorical_feature=SERIES_CATEGORICAL),
                     num_boost_round=cfg.load_forecast.rounds)


def forecast_rows(panel: Panel, boosters: dict[str, lgb.Booster], last_known: int, categories: dict,
    ...
    for tgt in TARGETS:
        ...
        part = rows[["series_id", "level", "org_code", "region_code", "profile_code", "horizon", "target_day_index"]].copy()
...
def add_fallback(fc_region: pd.DataFrame, small: Panel, shares: dict, last_known: int, cfg: ModelConfig) -> pd.DataFrame:
    ...
    for tgt in TARGETS:
        ...
        keep = rows[["series_id", "level", "org_code", "region_code", "profile_code", "horizon", "target_day_index"]].copy()
    ...
    for tgt in TARGETS:
        share = fb["series_id"].map(lambda sid: shares[sid][tgt]).to_numpy(float)
        region_pred = reg[f"pred_{tgt}"].reindex(pd.MultiIndex.from_arrays([fb["region_series"], fb["horizon"]])).to_numpy(float)
...
def queue_path(fc: pd.DataFrame, last_queue: pd.Series) -> np.ndarray:
    ...
    for i, (sid, start) in enumerate(zip(fc["series_id"].to_numpy(), q)):
        if sid != prev_sid:
...
def train_and_evaluate(hospital: Panel, region: Panel, cfg: ModelConfig, log=print) -> dict:
    ...
    def one_origin(origin: dt.date) -> pd.DataFrame:
        ...
        allq = pd.Series(np.concatenate([panel.queue[:, last], small_h.queue[:, last]]),
        ...
        actual_q = pd.Series(np.concatenate([panel.queue, small_h.queue]).tolist(),
                             index=pd.concat([panel.meta["series_id"], small_h.meta["series_id"]]))
        both["y_queue"] = [actual_q[s][int(ti)] for s, ti in zip(both["series_id"], both["target_day_index"])]
        both["b_last_queue"] = allq.reindex(both["series_id"]).to_numpy(float)
        both["eval_level"] = np.where(both["level"] == "region", "region × profile",
                                      np.where(both["series_id"].isin(small_h.meta["series_id"]),
                                               "hospital × profile (fallback)", "hospital × profile (modelled)"))
        queue_by_series[str(origin)] = both[["series_id", "eval_level", "horizon", "y_queue", "pred_queue", "b_last_queue"]]
        long = pd.concat([_long(both[both["eval_level"] == lvl], lvl) for lvl in both["eval_level"].unique()],
                         ignore_index=True)
        return long
    ...
    s, e = hospital.index_of(cfg.split.train_start), hospital.index_of(cfg.split.train_end)
    vol = pd.Series(modelled_h.registrations[:, s:e + 1].sum(axis=1), index=modelled_h.meta["org_code"]).groupby(level=0).sum()
    ...
    queue_table = error_table(q, ["origin", "eval_level", "bucket"], {"model": "pred", "last_value": "b_last_queue"}, bias=True)
    ...
    calendar_check = [{"date": str(r.date), "weekday": int(r.weekday), "registrations": float(r.registrations),
                       "weekday_median": float(r.weekday_median), "holiday_flag": bool(r.is_holiday_flag)}
                      for r in low.itertuples() if r.weekday <= 5]

    beats = []
    for r in pooled_all:
        cells = [c for c in per_origin if c["target"] == r["target"] and c["eval_level"] == r["eval_level"]]
        beats.append({"target": r["target"], "eval_level": r["eval_level"],
                      "wape_model": r["wape_model"], "wape_seasonal_naive": r["wape_seasonal_naive"],
                      "wape_mean_28d": r["wape_mean_28d"], "wape_mean_7d": r["wape_mean_7d"],
                      "beats_seasonal_naive": r["wape_model"] < r["wape_seasonal_naive"],
                      "beats_all_baselines": r["wape_model"] < min(r["wape_seasonal_naive"], r["wape_mean_28d"], r["wape_mean_7d"]),
    ...
    importance = {tgt: sorted(({"feature": f, "gain": float(g)} for f, g in
                               zip(b.feature_name(), b.feature_importance("gain"))), key=lambda x: -x["gain"])
                  for tgt, b in final.items()}
    return {
        "boosters": final,
        "categories": categories,
        "series": {"modelled_hospital_series": modelled_h.meta["series_id"].tolist(),
                   "region_series": region.meta["series_id"].tolist(),
                   "fallback_shares": setup["fallback_shares"]},
        "metrics": {
            "series": {"hospital_profile_total": len(hospital.meta), "hospital_profile_modelled": len(modelled_h.meta),
                       "hospital_profile_fallback": len(small_h.meta), "region_profile": len(region.meta),
...
def production_forecast(hospital: Panel, region: Panel, boosters: dict, categories: dict, series: dict,
                        cfg: ModelConfig) -> pd.DataFrame:
    ...
    both = pd.concat([fc, fb], ignore_index=True).sort_values(["series_id", "horizon"]).reset_index(drop=True)
    lastq = pd.Series(np.concatenate([panel.queue[:, last], small_h.queue[:, last]]),
                      index=pd.concat([panel.meta["series_id"], small_h.meta["series_id"]]))
    ...
    return both[["series_id", "level", "org_code", "region_code", "profile_code", "method", "origin_date",
                 "target_date", "horizon", "pred_registrations", "pred_hospitalizations", "pred_queue"]]
