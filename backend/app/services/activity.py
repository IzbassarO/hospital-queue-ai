...
def referrals(session: Session, org_code: str, profile_code: str, sort: Literal["risk", "wait"], limit: int,
              offset: int) -> Page[ReferralItem]:
    info = build_info(session)
    hospital_row(session, org_code, profile_code)
    params = {"org": org_code, "profile": profile_code, "start": info.date("test_start"),
              "end": info.date("test_end"), "limit": limit, "offset": offset,
              "high_risk": info.config["high_risk_threshold"]}
    where = ("pr.org_code = :org AND pr.profile_code = :profile "
             "AND pr.registration_date BETWEEN :start AND :end")
    ...
    """), params).all()
    items = [ReferralItem(hospitalization_code=r.hospitalization_code, registration_date=r.registration_date,
                          icd10_code=r.icd10_code, referral_purpose=r.referral_purpose,
                          pred_wait_days=round(r.pred_wait_days, 1), pred_refusal_prob=round(r.pred_refusal_prob, 4),
                          is_high_risk=r.pred_refusal_prob >= params["high_risk"], explanation=r.explanation)
             for r in rows]
...
def _decision(row: DecisionLog) -> Decision:
    return Decision(id=row.id, created_at=row.created_at, region_code=row.region_code, org_code=row.org_code,
                    profile_code=row.profile_code, recommendation_id=row.recommendation_id, action=row.action,
                    comment=row.comment, actor=row.actor)
...
def create_decision(session: Session, payload: DecisionCreate) -> Decision:
    ...
    if org.region_code != payload.region_code:
        raise ValidationError(f"hospital {payload.org_code!r} belongs to region {org.region_code!r}, "
                              f"not {payload.region_code!r}")
...
def list_decisions(session: Session, org_code: str | None, profile_code: str | None, limit: int,
                   offset: int) -> Page[Decision]:
    where = []
    ...
    rows = session.scalars(select(DecisionLog).where(*where)
...
def alerts(session: Session, region_code: str | None, limit: int, offset: int) -> Page[AlertItem]:
    ...
    trend_alert = (m.has_sufficient_data & (m.queue_trend_4w >= cfg["queue_trend_min_pct"])
    ...
    for r in rows:
        ...
        if r.load_index is not None and r.load_index >= cfg["load_index_min"]:
            ...
            reasons.append(f"Индекс нагрузки {_fmt(r.load_index)} ≥ {_fmt(cfg['load_index_min'], 0)}: "
                           f"место {r.region_rank} из {r.region_n_ranked} в регионе{backlog}")
        if (r.has_sufficient_data and r.queue_trend_4w is not None and r.queue_trend_4w >= cfg["queue_trend_min_pct"]
                and r.queue_now >= cfg["queue_trend_min_queue_now"]):
            reasons.append(f"Очередь растёт на {_fmt(r.queue_trend_4w)}% в неделю за последние 4 недели "
                           f"(сейчас {r.queue_now} чел.)")
        items.append(AlertItem(
            region_code=r.region_code, region_name=r.region_name, org_code=r.org_code, org_name=r.org_name,
            profile_code=r.profile_code, profile_name=r.profile_name, load_index=rnd(r.load_index, 1),
            status=r.status, queue_now=r.queue_now, backlog_days=rnd(r.backlog_days, 1),
            queue_trend_4w=rnd(r.queue_trend_4w, 1), refusal_rate_28d=rnd(r.refusal_rate_28d, 4), reasons=reasons,
        ))
