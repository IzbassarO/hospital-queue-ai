...
SYSTEM_PROMPT = {
    "ru": (
        ...
        "объяснение. Отвечай по-русски, коротко (3–6 предложений), только по этим фактам, ничего не выдумывай. "
        "Скажи, стоит ли специалисту согласиться с тем, что стационару нужно внимание, и почему. "
        "Это оценка внимания, а не назначение: не давай медицинских рекомендаций, не распределяй пациентов, "
        "не говори о койках, вместимости и занятости. Последняя фраза: решение и ответственность за специалистом."
    ),
    "kk": (
        ...
        "детерминирленген түсіндірме беріледі. Қазақша, қысқа (3–6 сөйлем), тек осы фактілер бойынша жауап бер, "
        "ештеңе ойдан шығарма. Маман стационарға назар қажет дегенмен келісуі керек пе, неге — соны айт. "
        "Бұл назар аудару бағасы, тағайындау емес: медициналық ұсыныс берме, пациенттерді бөлме, төсек-орын, "
        "сыйымдылық және толымдылық туралы айтпа. Соңғы сөйлем: шешім мен жауапкершілік маманда."
    ),
}
...
def ask(settings: Settings, request: AssistantRequest, timeout: float = 40.0) -> AssistantReply:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            # Groq sits behind Cloudflare, which rejects urllib's default signature
